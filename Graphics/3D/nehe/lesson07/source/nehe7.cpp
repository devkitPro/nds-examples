...
int main() {
	...
	while (1) 
	{
		...
		if (pressed & KEY_A) light = !light;

		int held = keysHeld();

		if (held & KEY_R) z-=0.02f;

		if (held & KEY_L) z+=0.02f;
		if (held & KEY_LEFT) xspeed-=0.01f;
		if (held & KEY_RIGHT) xspeed+=0.01f;
		if (held & KEY_UP) yspeed+=0.01f;
		if (held & KEY_DOWN) yspeed-=0.01f;
