...
* Build with F7.
* We don't use Visual Studio's output file vcxproj property for dependency tracking.. we let make handle it. So make is always run.

Addendum A. sln and vcxproj names
The template filenames are named arm9. This is a reasonable name, for reasons you may not understand yet.
However, you may wish to name the .sln and .vcxproj and the msvc project to match your project and directoryname. 
...
For instance:
  ...
  c:\test\myproject\myproject.vcxproj
  c:\test\myproject\myproject.nds
If you do rename these, then you must text-edit the .sln to reference the new vcxproj filename:
. Rename the sln and vcxproj files
. Textedit the sln. At about line 4, you will see:
    > "arm9", "arm9.vcxproj",
  "arm9" is the cosmetic name for the project. change it to "myproject"
  "arm9.vcxproj" is the link to the projectfile. change it to "myproject.vcxproj" -- whatever matches the vcxproj filename you just renamed to.
